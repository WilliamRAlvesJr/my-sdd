...
The first thing this run puts out is `▸ STEP 1: READ`, and it goes out before the first tool
call. Nothing has to be read to write it: what goes on that line is the argument you were
handed, and you have it before you have opened anything. So a look around to get your bearings
is not what comes before the step, it is the step, and marks written once the reading is done
are a report of a run rather than a trace of one.

Every file named here is opened by the step that names it, and by no step before it. This one
hands over a `spec.md` and stops short of it in four different places, so a file fetched early
is as often as not a file fetched for a run that ends before using it.
...
everything after the `·` is written in the language the request came in.** The request is in
front of you when you open STEP 1, so read which language it is in before you write the first
mark, and stay in it to the last: a request that says `crear una invitación` closes STEP 1 on a
line in Spanish, and one that says `criar um board` closes it on a line in Portuguese, every
example in this file being in English changing neither. The language the request came in is the
language of the conversation, the same split R45 makes inside a `scenario`, and it decides
every mark, every message and the report, whatever language this file is written in. Literal is
the mark, the word `STEP`, the number and, on the `▸`, the step's name copied from its own
heading, which is `READ`, `WORK OUT THE BEHAVIORS`, `PROPOSE AND STOP`, `FILL IN THE TEMPLATE`
and `REPORT`, those five spellings and no translation of any of them. You tend to
...
neither mark announces the step that comes next. The empty item is the one you most want to
trade for something you did find, and what turns up in its place is always true and never
something the line asked for: the stack the project is built with, the version it is on, the
folder you are about to name. The count still reads right and the item that fell out is gone
with nothing said. Each step's `In` and `Out` is already written
...
A project with nothing built in it closes on those same four items, since an item that came out
empty says so instead of dropping off and nothing else takes its place:

```
◂ STEP 1 · create an invite; no convention to inherit; nothing around it, the project has no feature yet; the security dependency is declared and nothing configures it, so every call answers 401
```

The request behind that line came in English, and that is the only reason the line is in
English. What carries over from it is the four items and nothing else: the words are this run's
request, in this run's language.
...
Each of the four is a file you opened. Nothing on the `◂` comes from what a project of this
kind usually holds, and a step that closes without having opened anything has reported four
findings it doesn't have.
...
This is the run's first tool call. There is nothing to size up before it, and the look around
the project you would open with is this same reading done in a worse order.

The answer is not one of the four things the `Out` names: yes ends the step by R17, and no
leaves nothing to hand to STEP 2. It is the first thing you look for and so the first thing you
want to report, and on the `◂` it takes the place of an item that belongs there, which is how
four things read come out as three.
...
under that root, this file and whatever sits next to it being outside it like anything else. A
file of either name outside that root belongs to whoever owns the machine, or to
...
Go through what the project declares before writing this item, and say what each of those does
with nothing configuring it. Declared is not configured: a dependency nobody set up is on with
whatever it defaults to, no line of the source mentions it, and finding no configuration for it
is what confirms the obstacle rather than what clears it. *Nothing in the way*, written before
that walk, is the answer you had before reading anything, and a project with no feature in it
yet is exactly where it is wrong, since there what it declares is the only thing there is to
read.
...
anything. Nothing this step calls changes the project either: no folder created, no file
written, no command that leaves something behind. The folder is named in STEP 3 and made in
STEP 4, and a run that makes it here has built the shape of an answer the user has not
confirmed.

Nothing a later step will need is fetched here either: not the skeleton, not the validator, not
the interpreter that runs it. Four of this step's checks end the run outright, so anything
fetched ahead of them was fetched for a `feature` nobody is going to write, and the pull is to
read that as being ready rather than as work done early. The step reads the project, and the
project is the only thing under it.
...
**R41 ·** Write a `spec.md` inside each folder STEP 3 settled, copied from `TEMPLATE.md`, next
to this file. That file is opened here and nowhere earlier: a step that reads the skeleton
before there is a confirmed list has read the shape of an artifact it is not writing yet.
